...
export default function ModalCreateObject({
  ...
  useEffect(() => {
    ...
    const getObjectFieldsOfType = Object.keys(objectCreate.value).map(
        objectField => {
          return {
      ...
      getRequiredFields = getObjectFieldsOfType.filter(
        objectField => !!objectField?.required
      )
  ...
  const tryFieldValidate = (fieldName, fieldValue) => {
      ...
      const findRequiredField = getRequiredFields.find(
        requiredField => requiredField.fieldName === fieldName
      )
      ...
      setErrorsFieldsBeforeValidate(beforeErrorsFields => {
        ...
        if (fieldValue.toString().length === 0)
          newErrors[fieldName] = "Поле обзятельно для заполнения!"
    ...
    tryValidateForm = () => {
      if (JSON.stringify(errorsFieldsValidate) === "{}") {
        ...
        Object.keys(errorsFieldsValidate).map(errorField => {
  ...
  const sortObjectInputFields = useMemo(() => {
      ...
      const getOfInputType = Object.keys(objectCreate.value).map(fieldValue => {
          return {
            ...objectCreate.fields[fieldValue],
            field: fieldValue,
          }
        }),
        getInputTypes = getOfInputType.map(field => field.input)
    ...
    getErrorsFields = useMemo(() => {
      return Object.keys(objectCreate.value)
        .map(fieldValue => {
          return {
            ...objectCreate.fields[fieldValue],
            field: fieldValue,
  ...
  useEffect(() => {
    ...
    if (!!objectCreate?.error) {
      setFormError(objectCreate?.error?.response?.data?.error?.message)
  ...
  const nextStep = async () => {
    ...
    finishStep = async () => {
      ...
      if (objectCreate.type === "sale-business") {
        if (objectCreate.createdObject?.tenantsInfo?.length > 0) {
          const createTentant = objectCreate.createdObject?.tenantsInfo?.filter(
            tentant => tentant.type === "create"
          )

          await createTentantsInCard(
            createTentant.map(tentant => {
              const newTentant = {
                ...tentant,
              }
    ...
    prevStep = () => {
      if (
        stageForm === "objectCreated" &&
        objectCreate.type === "sale-business"
      ) {
  ...
  const close = () => {
    if (objectCreate.createdObject !== null) {
      const confirmDelete = confirm(
        "Если вы закроете данные будут удалены, вы уверены?"
      )

      if (confirmDelete)
        dispatch(deleteCreatedObject(objectCreate.createdObject.id))
    ...
    <>
      ...
      >
        <div className="flex px-[24px]">
          <DialogTitle sx={{ paddingX: 0, flex: "1" }}>
            {" "}
            Создание обьекта{" "}
          </DialogTitle>
        ...
        <DialogContent>
          ...
          <Stepper activeStep={step}>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-3 gap-5">
                      {sortObjectInputFields.length > 0 &&
                        sortObjectInputFields.map((createField, i) => {
                          const createInput = createField,
                            createInputValue =
                              objectCreate.value[createInput.field]

                          return (
                            <>
                              {(createInput.input === "text" ||
                                createInput.input === "number" ||
                                createInput.input === "textarea") && (
                                <TextField
                                  variant="outlined"
                              ...
                              {(createInput.input === "file" ||
                                createInput.input === "files") && (
                              ...
                              {createInput.input === "checkbox" && (
                                <>
                                  <FormControlLabel
                                    control={
                                      <Checkbox
                                        value={checkboxes[createInput.field]}
                                        onClick={e => {
            ...
            <Step
              sx={{
                display:
                  objectCreate.type === "sale-business" ? "block" : "none",
              }}
            >

...
import { archiveCard } from '../api/api.js';
...
export default function ModalWindow({ isCreate }) {
  ...
  const update = async id => {
    const getCurrentTitle = await getCurrentCard(object.slug).then(
      r => r.data.title,
    );
    ...
    const copyObject = {
      ...
      layoutImages: await Promise.all(
        object.layoutImages.map(async layoutImage => {
          return {
            ...layoutImage,
            file: (
              await axios.get(layoutImage.url, {
                responseType: 'blob',
              })
            ).data,
          };
  ...
  const copyObject = async id => {
    try {
      const getCurrentObject = await getCurrentCard(object.slug).then(
        r => r.data,
      );
  ...
  const handleArchive = async () => {
    const response = await archiveCard(object.id);

    if (response > 300) return;
  ...
  return (
    <>
      <div
        className={`${
          isOpen ? 'block' : 'hidden'
        } top-0 right-0 left-0 bottom-0 opacity-60 bg-black fixed`}
        onClick={e => clickOutside(e)}
      ></div>
      <div
        className={`${
          isOpen ? 'block' : 'hidden'
        } left-[50%] translate-x-[-50%] fixed h-max w-max flex items-center justify-center`}
      >
        <div
          className={
            'bg-white  relative w-[1000px] max-h-[85vh] px-[32px]  overflow-y-auto'
          }
        >
          <div className={'pt-[40px] flex flex-col gap-4 pb-[20px]'}>
            <div className={'flex justify-between items-center'}>
              <p className={'font-bold text-red-700 text-left text-3xl'}>
                {infoProject[object.type]}
              </p>
              <div className={'flex gap-4'}>
                ...
                <div>
                  <button
                    className={
                      'text-[20px]  right-24 my-[24px] leading-[28px] bg-[#144728] px-[20px] py-2 text-white rounded-[5px] md:hover:bg-[#1E653A] shadow-lg active:bg-[#0B2716] duration-300 font-[300] font-inter'
                    }
                    onClick={() => {
                      navigate('/pdf');
                    }}
            ...
            <div className={'flex gap-2.5'}>
              <Tag
                title={`${object.type === 'rent' ? 'Аренда/мес' : 'Цена:'}`}
                subName={'global'}
                name={'price'}
              />
              <Tag
          ...
          <div className={'flex flex-col w-full gap-4'}>
            ...
            <div className={'flex items-center'}>
              <p>Вытяжка</p>
              <Input inputType="checkbox" path="info.hood" />
            </div>
            <div className={'flex items-center'}>
              <p>Зона погрузки/разгрузки:</p>
              <Input inputType="checkbox" path="zone" />
            </div>

            <div className="flex items-center">
              <p>Новый обьект</p>
              <Input inputType="checkbox" path="isNew" />
            </div>
            <div className="flex items-center">
              <p>Новая цена</p>
              <Input inputType="checkbox" path="isNewPrice" />
            </div>
            ...
            {stateWindow === 'sale-business' && (
              <>
                ...
                <Tag
                  type={'number'}
                  title={'Месячный арендный поток:'}
                  subName={'mouth'}
                  name={'globalRentFlow'}
                />
                <Tag
                  type={'number'}
                  title={'Годовой арендный поток:'}
                  subName={'year'}
                  name={'globalRentFlow'}
                />
                <Tag
          ...
          {object.tentantLogo !== 'delete' &&
            object.tentantLogo.length !== 0 && (
              <div className="relative flex gap-2 group">
                <img
                  className="max-h-[300px] h-full w-full object-center object-cover"
                  src={
                    !!object.tentantLogo?.url
                      ? object.tentantLogo.url
                      : !!object.tentantLogo.match('http|https|blob')
                      ? object.tentantLogo
                      : location.origin + '/public/' + object.tentantLogo
                  }
                />
          ...
          <div className={'flex gap-2'}>
            <button
              ...
              className={
                'text-[20px] my-[24px] leading-[28px] bg-[#144728] px-[40px] py-[14px] text-white rounded-[5px] md:hover:bg-[#1E653A] shadow-lg active:bg-[#0B2716] duration-300 font-[300] font-inter'
              }
            >
              {changeIsLoading ? (

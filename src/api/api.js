...
export const editTentant = async ({ id, name, category, logo }) =>
  await instanceApi.patchForm(`/tentant/${id}`, { name, category, logo });
export const deleteTentant = async id =>
  await instanceApi.delete(`/tentant/${id}`);
export const addTentant = async ({ name, category, logo }) =>
...
export const getArchiveCards = async () =>
  await instanceApi.get('/objects/archive');
...
export const getArendators = async () =>
  (await instanceApi.get('/tentants')).data;
// export const getArendators = async () => {
...
export const removeTentantOfObject = async (tentantsId, objectId) =>
  await instanceApi.delete(`/object/remove-tentant/${objectId}`, {
    data: tentantsId,
  });
...
export const createTentantsInCard = async (tenstantsInfo, id) =>
  await instanceApi.post(`/object/add-tentant/${id}`, {
    tentants: tenstantsInfo,
  });
export const updateTentantsInCard = async (tenstantsInfo, id) =>
  await instanceApi.put(`/object/update-tentant/${id}`, [...tenstantsInfo]);
...
export const updateCard = async (id, data) => {
  const formUpdate = objectToFormData({
    ...
    priceSaleGlobal: data.price.sale.global,
    priceSaleSquare: data.price.sale.square,
    ...
    tentantLogo:
      typeof data.tentantLogo === 'string'
        ? data.tentantLogo.length > 0
          ? data.tenstantsInfo
          : undefined
        : data.tentantLogo.file,
